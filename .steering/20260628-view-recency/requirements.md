...
- 連続する閲覧イベントを、セッション単位で閲覧順位を定義する
- セッション境界の定義（例: 一定時間の無操作）は追加時に確定する。現状では日付単位でセッションとしてまとめらることを想定している。

**`frequency_mode="day"`**（本タスクでは実装しない）
- 現状の `frequency` は閲覧イベント数（view freq）
- day freq は閲覧した日数（ユニーク日付数）を frequency として使う指標
- day freq は**日序数（`_SEQUENCE_COL`）の `nunique`** で求まり、日序数は day/view 両モードで常に存在するため、**`recency_mode` および高解像度キーと直交**する（高解像度キー不要）
- 本タスクで recency/frequency ビルダーを `_recency.py` に外出しする設計を確立し、`frequency_mode` 追加時はビルダー内の frequency 集約エントリを切り替えるだけ（groupby を増やさない）で実装できるようにする
...
`recency_mode="view"` 時、`unit` は recency 計算に使用しない（`unit` は recency 軸のビン幅であり、view では recency がランクのためビン化しない）。

> **将来メモ（本タスクでは実装しない）**: ビン幅を軸ごとに指定できるよう、将来的に `unit` を
> **`recency_unit` / `frequency_unit`** に分割するのが自然。frequency 軸のビン化はビルダー末尾の
> 純粋後処理（`frequency = (frequency - 1) // frequency_unit + 1`）で足せ、`recency_mode` /
> `frequency_mode` と直交する。今回はメモのみで、`unit` の現仕様（recency 軸ビン幅）は変更しない。

### AC-5: 拡張しやすい内部設計

将来の `recency_mode="session"` および `frequency_mode` 追加を見越し、以下を満たす設計とする。

- recency/frequency の計算ロジックを **`_recency.py` の純粋関数（`build_day_rf` / `build_view_rf`）に外出し**し、`_build_ui_rf_df` は `recency_mode` で振り分けるだけのディスパッチャにする
- `recency_mode` の分岐は 1 箇所（ディスパッチャ）に集約し、新 mode 追加はビルダー関数の追加で済むようにする
- ビルダーは `fit_rolling` の日窓フィルタと両立すること（窓フィルタは日序数のまま、view 順位はフル解像度キーで窓ごとに計算）
- 将来 `frequency_mode` パラメータを追加する際、ビルダー内の frequency 集約エントリ（`count`↔`nunique`）の切り替えで実装でき、groupby を増やさないこと（本タスクでは `frequency_mode` パラメータは追加しない）

### AC-6: テストの追加

- `_recency.py` のビルダー（`build_view_rf` / `build_day_rf`）の単体テストで下記テスト例 1〜3 を直接検証すること
- `recency_mode="day"` でデフォルト挙動が変わらないこと
- `recency_mode="view"` で `fit()` / `predict()` / `transform()` / `fit_rolling()` が動作すること
...
6. `fit()` / `predict()` / `transform()` / `fit_rolling()` が `recency_mode="view"` でも動作する
