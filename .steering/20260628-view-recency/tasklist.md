...
- [x] **T02** `__init__()` に `recency_mode: str = "day"` を追加する（`scorer.py` 50 行付近）
...
- [x] **T03** Phase 1 の回帰確認: `uv run pytest -q`
...
- [x] **T04** `_time_utils.py` に `normalize_view_key()` を追加する（design §2）
...
- [x] **T05** `_to_internal()` に view モード時の `_VIEW_KEY_COL` 生成を追加する（`scorer.py` 265 行付近）
...
- [x] **T06** `transform()` に同様の `_VIEW_KEY_COL` 生成を追加する（`scorer.py` 944 行付近）
  - `df_log[self._SEQUENCE_COL]` 上書きの直前に同パターンで挿入

- [x] **T07** Phase 2 の回帰確認: `uv run pytest -q`
...
- [x] **T08** `src/rfscorer/_recency.py` を新規作成する（design §3）
...
- [x] **T09** `_build_ui_rf_df` をディスパッチャに置き換える（`scorer.py` 1596 行付近）
...
- [x] **T10** Phase 3 の回帰確認: `uv run pytest -q`
  - 既存テストが全パスすること（`recency_mode="day"` のディスパッチャ化が挙動を変えていないことの担保）

### Phase 4: 周辺箇所の更新

- [x] **T11** `show()` の Model セクションに `recency_mode` を追記する（1506 行付近）
  ```python
  print(f"  recency_mode     : {self.recency_mode}")
  print(f"  recency_limit    : {self.recency_limit}")
  ```

- [x] **T12** `save_zip()` の `metadata` dict に `recency_mode` を追加する（1317 行付近）
  ```python
...
- [x] **T13** `__init__()` の docstring に `recency_mode` パラメータ説明を追加する
  ```
...
- [x] **T14** `fit()` の `ref` パラメータ説明に注記を追加する
  - `recency_mode="view"` 時は `ref` が無視される旨を1行追記

- [x] **T15** `transform()` の `ref` パラメータ説明に注記を追加する
  - `fit()` と同じ方針で1行追記

### Phase 5: テストの追加

- [x] **T16** `tests/test_recency.py` を新規作成し、`build_view_rf` / `build_day_rf` の単体テストを追加する
...
- [x] **T17** `tests/test_scorer.py` に `TestViewRecency` クラスを追加する（統合レベル）
...
- [x] **T18** テスト実行: `uv run pytest tests/test_recency.py tests/test_scorer.py::TestViewRecency -v`
  - 全ケースパス

### Phase 6: ドキュメントの更新

- [x] **T19** `docs/functional-design.md` を更新する
...
- [x] **T20** リントを実行する: `uv run ruff check .`

- [x] **T21** フォーマットを確認する: `uv run ruff format --check .`

- [x] **T22** 全テストを実行する: `uv run pytest`
