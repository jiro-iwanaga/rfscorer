...
- データ準備の補助
  ...
  - `split_by_date(df, target_date, observation_days=28, evaluation_days=7, time_col="datetime")` で単一の閲覧履歴 DataFrame を観測ログ・評価ログに分割可能（戻り値は `(df_obs, df_eval)` のタプル）。`observation_days=N` で観測期間は N 時間単位（`target_date` 含む `[target - N + 1, target]`）、`evaluation_days=N` で評価期間は N 時間単位（`[target + 1, target + N]`）の対称ウィンドウ
...
| 推薦精度の評価（`evaluate`） | 推薦結果と評価期間のイベント履歴を比較し、各順位カットオフでの precision・recall・f1 と、df_rec 内で達成可能な最大ヒット数で正規化した recall_norm・f1_norm を返す |
| 推薦スコアリング（`transform`） | 観測期間の閲覧履歴 DataFrame の各 user×item ペアに最新度・頻度・商品選択確率・推薦順位を付与して返す。`recency_limit`・`frequency_limit` を超える値は確率参照時に上限にクランプ（出力カラムは元値を保持） |
