...
EC サイトやコンテンツプラットフォームでは、ユーザーの閲覧ログに基づいて商品推薦を行いたいというニーズがある。  一般的なブラックボックス型の推薦モデルは、スコアの根拠を説明しにくく、運用・デバッグが困難な場合がある。
...
- `fit()` に渡す引数（推奨）
  - `df_obs`: 観測期間の閲覧ログ DataFrame。同一ユーザー × 商品の組み合わせが複数行存在することを想定（リピート閲覧）
...
- データ準備の補助
  ...
  - `split_by_date(df, target_date, observation_days=28, gt_days=7, time_col="datetime")` で単一の閲覧ログ DataFrame を観測ログ・正解ログに分割可能（戻り値は `(df_obs, df_gt)` のタプル）。`observation_days=N` で観測期間は N 時間単位（`target_date` 含む `[target - N + 1, target]`）、`gt_days=N` で正解期間は N 時間単位（`[target + 1, target + N]`）の対称ウィンドウ
...
| 推薦スコアリング（`transform`） | 観測期間の閲覧ログ DataFrame の各 user×item ペアに最新度・頻度・商品選択確率・推薦順位を付与して返す。`recency_limit`・`frequency_limit` を超える値は確率参照時に上限にクランプ（出力カラムは元値を保持） |
