...
本パッケージは、解釈可能な推薦スコアリング手法を提供する。Recency（最新度: ユーザーが商品を最後に閲覧してからの経過時間）と Frequency（頻度: ユーザーが商品を閲覧した回数）の2つの行動シグナルのみを用い、ユーザー × 商品ペアごとに商品選択確率を推定する。各ペアに付与される商品選択確率は選好スコアとして機能し、得られる行列は評価値行列（rating matrix）と類似した構造を持つため、協調フィルタリングや機械学習を用いた推薦モデルの入力（特徴量）としても活用できる。手法の理論的基盤は以下の学術論文に基づく。
...
| 個別確率取得（`predict`） | 指定した最新度 $r$・頻度 $f$ に対応する商品選択確率を1件返す。1次元モデルでは片方の引数のみ参照（`kind='mr'/'er'` は $r$ のみ、`kind='mf'/'ef'` は $f$ のみ）。`recency_limit`・`frequency_limit` を超える値は上限にクランプ |
...
scorer.show()                            # データ統計・相関係数・確率テーブルを表示
...
scorer.evaluate(df_rec, df_gt)                        # 評価
...
# pickle 形式で保存・ロード（サーバーデプロイ向け）
scorer.save("models/scorer.pkl")         # ファイルパス指定
scorer.save("models/")                   # ディレクトリ指定（rfscorer.pkl として保存）
scorer.save()                            # カレントディレクトリに rfscorer.pkl
scorer_loaded = RecencyFrequencyScorer.load("models/scorer.pkl")

# zip アーカイブで保存・ロード（研究者間の共有・成果物管理向け）
scorer.save_zip("artifacts/scorer.zip")  # pickle + CSV + PNG を一括保存
scorer_loaded_zip = RecencyFrequencyScorer.load_zip("artifacts/scorer.zip")
```
