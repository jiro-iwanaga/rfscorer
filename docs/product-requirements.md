...
- データ準備の補助
  ...
  - `split_by_date(df, target_date, observation_days=28, gt_days=7, time_col="datetime")` で単一の行動履歴 DataFrame を観測データ・正解データに分割可能（戻り値は `(df_obs, df_gt)` のタプル）。`observation_days=N` で観測期間は N 時間単位（`target_date` 含む `[target - N + 1, target]`）、`gt_days=N` で正解期間は N 時間単位（`[target + 1, target + N]`）の対称ウィンドウ
