...
- `fit()` に渡す引数（推奨）
  ...
  - `df_eval`: 正解期間のイベント履歴 DataFrame（閲覧・購買・CV など推定対象のイベント）。`df_obs` と同じカラム構成
...
- データ準備の補助
  ...
  - `split_by_date(df, target_date, observation_days=28, evaluation_days=7, time_col="datetime")` で単一の閲覧ログ DataFrame を観測ログ・正解ログに分割可能（戻り値は `(df_obs, df_eval)` のタプル）。`observation_days=N` で観測期間は N 時間単位（`target_date` 含む `[target - N + 1, target]`）、`evaluation_days=N` で正解期間は N 時間単位（`[target + 1, target + N]`）の対称ウィンドウ
