...
| 1次元経験的商品選択確率の推定（`er` / `ef`） | `fit()`・`fit_date()` または `fit_period()` 時に自動計算。最新度方向（`er`）・頻度方向（`ef`）の経験的周辺確率を1次元の dict / DataFrame として保持する |
...
| `er_probability_` | 1次元経験的商品選択確率（`er`）。`pd.DataFrame`（カラム: `recency`, `probability`）。`fit()`・`fit_date()` または `fit_period()` 後にアクセス可能 |
| `ef_probability_` | 1次元経験的商品選択確率（`ef`）。`pd.DataFrame`（カラム: `frequency`, `probability`）。`fit()`・`fit_date()` または `fit_period()` 後にアクセス可能 |
