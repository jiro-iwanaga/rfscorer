...
| `export_probability_csv(kind='emp', path=None)` | 商品選択確率を CSV ファイルに書き出すメソッド。`kind` に `'emp'`・`'er'`・`'ef'`・`'mono'`・`'mr'`・`'mf'`・`'mrc'`・`'mfc'`・`'mcc'`・`'all'` を指定する（`'all'` は全モデルを結合した1ファイルを生成）。`path=None` でカレントディレクトリに `{kind}_probability.csv` を生成。ディレクトリを指定するとその中に同名ファイルを生成。`fit()` 後（最適化モデルの種類は対応する `optimize()` 後）に利用可能 |
| `plot_probability_surface(kind='emp', title=None, figsize=(6, 5), fontsize=12, recency_label='recency', frequency_label='frequency', probability_label='probability', path=None)` | 商品選択確率を3次元ワイヤーフレームで可視化し `matplotlib.figure.Figure` を返すメソッド。軸ラベル・タイトル・図サイズ・フォントサイズを指定可能。日本語ラベルには `rfscorer[ja]` が必要。`fit()` 後（`kind='mono'/'mrc'/'mfc'/'mcc'` の場合は `optimize()` 後）に利用可能。`kind='mr'`・`'mf'`・`'er'`・`'ef'` は1次元モデルのため `ValueError` を送出する（折れ線表示には `plot_marginal_probability()` を使用する）。`title=None` でデフォルトタイトル（`kind` に応じて `"Empirical"`・`"Monotonic"`・`"Monotonic (Recency Convex)"` など）を自動設定。`title=""` でタイトルなし。`path=None` で保存しない。ディレクトリを指定すると `surface_{kind}_probability.png` を生成。ファイルパスを指定すると指定名で保存 |
| `plot_marginal_probability(kind='er', title=None, figsize=(5, 4), fontsize=12, axis_label=None, probability_label='probability', path=None)` | 最新度または頻度の一方向の商品選択確率を折れ線グラフで可視化し `matplotlib.figure.Figure` を返すメソッド。プロットする軸は `kind` から自動判定される（`kind='er'/'mr'/'rboth'` → recency 軸、`kind='ef'/'mf'/'fboth'` → frequency 軸）。`kind='er'` で経験的 recency のみ、`kind='ef'` で経験的 frequency のみ、`kind='mr'` で単調（凸）recency のみ、`kind='mf'` で単調（凹）frequency のみ、`kind='rboth'` で er と mr を重ねて表示（legend 付き）、`kind='fboth'` で ef と mf を重ねて表示（legend 付き）。`title=None` でデフォルトタイトル（`kind` に応じて `"Empirical Recency"`・`"Monotonic Recency (Convex)"`・`"Recency: Empirical & Monotonic"` など）を自動設定。`title=""` でタイトルなし。`axis_label=None` のとき x 軸ラベルは `kind` に応じて `"recency"` または `"frequency"` に自動設定される。日本語ラベルには `rfscorer[ja]` が必要。`fit()` 後に利用可能（`kind='mr'/'mf'/'rboth'/'fboth'` の場合は対応する `optimize()` 後も必要）。`path=None` で保存しない。ディレクトリを指定すると `marginal_{kind}_probability.png` を生成。ファイルパスを指定すると指定名で保存 |
| `optimize(kind='mono', eps=0.0, verbose=False)` | `fit()` の結果を用いて、RF 制約付きの最適化商品選択確率を推定するメソッド。`kind='mono'`（単調性のみ）・`'mrc'`（単調性 + Recency 凸性）・`'mfc'`（単調性 + Frequency 凹性）・`'mcc'`（単調性 + 両凹凸性）・`'mr'`（1次元 Recency）・`'mf'`（1次元 Frequency）を指定する（長名エイリアスも使用可）。`eps > 0` で狭義単調性を適用。`verbose=True` で最適化計算の情報を標準出力に表示する |
