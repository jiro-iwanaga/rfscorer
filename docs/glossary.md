...
| `evaluate(df_rec, df_eval, order=1, ...)` | 推薦結果と評価期間のイベント履歴 `df_eval` を比較し評価指標を返すメソッド。戻り値カラムは `order`, `n_recommended`, `n_hit`, `precision`, `recall`, `f1`, `recall_norm`（df_rec 内で達成可能な最大ヒット数で正規化した recall）, `f1_norm`（recall_norm を用いた f1）。`user_col`・`item_col` は省略すると `__init__` の設定値を使用する |
