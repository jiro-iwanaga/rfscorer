...
- テストコードは `tests/` に配置する
- `pytest` を使用する
- `fit()`・`predict()`・`transform()`・`evaluate()`・`optimize()`・`plot_probability_surface()`・`plot_marginal_probability()`・`export_probability_csv()`・`save()`・`load()`・`save_zip()`・`load_zip()` の正常系・異常系をカバーする
